        ...
        {top3.length === 0 ? (
          ...
          top3.map((entry) => (
            ...
            >
              ...
              <Link to={`/users/${entry.athlete_id}`} onClick={(e) => e.stopPropagation()} style={{ flex: 1, fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: "inherit", textDecoration: "none" }}>
                {entry.athlete_name}
              </Link>
              {entry.activity_id
                ? <Link to={`/activities/${entry.activity_id}`} onClick={(e) => e.stopPropagation()} style={{ fontWeight: 700, fontVariantNumeric: "tabular-nums", color: "var(--text)", textDecoration: "none" }}>{fmt(entry.best_time)}</Link>
                : <span style={{ fontWeight: 700, fontVariantNumeric: "tabular-nums", color: "var(--text)" }}>{fmt(entry.best_time)}</span>
              }
            </div>
          ))
        )}
