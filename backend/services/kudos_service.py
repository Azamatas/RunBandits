...
def give_kudos(db: Session, activity_id: int, user_id: int) -> KudosResponse:
    ...
    if existing:
        ...
        raise ConflictError("Already gave kudos")
    ...
    return KudosResponse(kudos_count=len(activity.kudos), user_has_kudos=True)


def remove_kudos(db: Session, activity_id: int, user_id: int) -> KudosResponse:
    ...
    if not kudos:
        ...
        raise NotFoundError("Kudos not found")
    db.delete(kudos)
    db.commit()
    activity = activity_service.get_activity(db, activity_id, user_id)
    if activity:
        ...
        return KudosResponse(kudos_count=len(activity.kudos), user_has_kudos=False)
    logger.warning(f"Activity {activity_id} not found after removing kudos")
    return KudosResponse(kudos_count=0, user_has_kudos=False)
