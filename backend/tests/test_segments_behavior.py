...
class TestSegmentCreation:
    def test_creating_segment_returns_auto_calculated_distance(self, client, user_a):
        _, headers = user_a
        resp = client.post("/segments/", json={"name": "Vondelpark Loop", "polyline": VONDELPARK}, headers=headers)
        assert resp.status_code == 200
        assert resp.json()["distance"] > 0

    def test_segment_appears_in_list_after_creation(self, client, user_a):
        _, headers = user_a
        client.post("/segments/", json={"name": "My Segment", "polyline": VONDELPARK}, headers=headers)
    ...
    def test_cannot_create_segment_without_login(self, client):
        resp = client.post("/segments/", json={"name": "X", "polyline": VONDELPARK})
...
class TestAutoMatching:
    def test_running_through_segment_creates_effort(self, client, user_a):
        ...
        seg = client.post("/segments/", json={"name": "Vondelpark", "polyline": VONDELPARK}, headers=headers).json()
        ...
        efforts = client.get(f"/segments/{seg['id']}/efforts", headers=headers).json()
    ...
    def test_running_different_route_creates_no_effort(self, client, user_a):
        ...
        seg = client.post("/segments/", json={"name": "Amstel Sprint", "polyline": AMSTEL}, headers=headers).json()
...
class TestLeaderboard:
    def test_faster_athlete_ranks_higher(self, client, user_a, user_b):
        ...
        seg = client.post("/segments/", json={"name": "Vondelpark", "polyline": VONDELPARK}, headers=headers_a).json()
        ...
        board = client.get(f"/segments/{seg['id']}/leaderboard", headers=headers_a).json()
    ...
    def test_only_best_time_counts_per_athlete(self, client, user_a):
        ...
        board = client.get(f"/segments/{seg['id']}/leaderboard", headers=headers).json()
...
class TestPersonalEfforts:
    def test_my_efforts_shows_only_mine(self, client, user_a, user_b):
        ...
        efforts_a = client.get(f"/segments/{seg['id']}/efforts", headers=headers_a).json()
        efforts_b = client.get(f"/segments/{seg['id']}/efforts", headers=headers_b).json()
